...
describe("HomePage Component", () => {
    ...
    it("If add to cart button works", async () => {
        render(
            <MemoryRouter>
                <HomePage cart={[]} loadCart={loadCart} />
            </MemoryRouter>
        ...
        const quantityElement1 = within(productContainers[0]).getByTestId("product-quantity")
        await user.selectOptions(quantityElement1, "2")
        await user.click(addToCartButton1)

        const addToCartButton2 = within(productContainers[1]).getByTestId("add-to-cart-button")
        const quantityElement2 = within(productContainers[1]).getByTestId("product-quantity")
        await user.selectOptions(quantityElement2, "3")
